...
class System:
    ...
    def _setup_user_equipments(self, user_equipments_config):
        if isinstance(user_equipments_config, list):
            for ue in user_equipments_config:
                self._user_equipment_manager.create_user_equipments(
                    ue["mode"], ue["x"], ue["y"], ue["number_of_ues"],
                    self._frequency, self._bandwidth, self._tx_power)
        elif isinstance(user_equipments_config, dict):
            self._user_equipment_manager.create_user_equipments(
                user_equipments_config["mode"], user_equipments_config["upper_x_bound"],
                user_equipments_config["upper_y_bound"], user_equipments_config["number_of_ues"],
                self._frequency, self._bandwidth, self._tx_power)
        else:
    ...
    def simulate_scenario(self, scenario_name):
        ...
        scenario_config = settings["scenarios"][scenario_name]

        self._setup_base_stations(scenario_config["base_stations"])
        self._setup_user_equipments(scenario_config["user_equipments"])
        self._configure_basics()

        self._capacity = self._initialize_capacity_calculator(scenario_config["capacity_calculator"])
        self._throughput = self._capacity.calculate_downlink_aggregate_throughput_over_number_of_slots()
